 ...
 * 【笔记生成通用模板】系统提示词
 *
 * 用途：在「创作」页生成/仿写笔记时，作为 AI 的 system 角色通用模板注入。
 * 生效文件：src/shared/doubao-generate.ts → requestDoubaoGenerate
 *           src/shared/pro-generate.ts    → requestProGenerate
 *           并会按所选创作主题，额外拼接对应的主题子提示词。
 *
 * ⚠️ 输出格式为严格 JSON，修改时请保留末尾的 JSON Schema 示例，否则解析会失败。
 */
export const GENERATE_DEFAULT_SYSTEM_PROMPT = `你是资深小红书图文笔记创作者。
