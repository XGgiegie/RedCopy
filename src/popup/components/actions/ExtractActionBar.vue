...
import type { AnalysisProvider, DeepSeekModel, DoubaoModel } from '../../../shared/ai-settings'
import {
...
import InfoTip from '../layout/InfoTip.vue'
...
const analyzeDisabledReason = computed(() => {
  if (props.mode !== 'detail') return '请先点击历史记录进入详情'
  if (!props.hasNote) return '当前记录无笔记内容'
  if (props.isGenerating) return '正在生成类似笔记'
  if (!props.isAiConfigured) {
    return `请先配置${props.analysisProviderLabel} API Key`
  }
  return ''
})

const extractTooltip = computed(() => {
  if (props.isNotePage) return '提取当前笔记到历史记录'
  return '请先打开小红书笔记详情页'
})
...
<template>
  <div class="extract-actions">
    <NTooltip v-if="mode === 'list'" trigger="hover" :disabled="isNotePage">
      <template #trigger>
        <NButton
          type="primary"
          block
          ...
          <template #trigger>
            ...
            >
              <svg viewBox="0 0 16 16" fill="currentColor" aria-hidden="true">
                <path
                  d="M8 1.2 1.5 13h13L8 1.2Zm0 3.2.9 1.8H7.1L8 4.4ZM7.25 8h1.5v3.5h-1.5V8Z"
                />
              </svg>
            </button>
          </template>
